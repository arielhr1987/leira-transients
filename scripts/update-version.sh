...
PLUGIN_FILE="leira-transients.php"
...
# Update define('LEIRA_TRANSIENTS_VERSION', ...) (preserve spacing)
sed -E -i '' "s/(define\([[:space:]]*'LEIRA_TRANSIENTS_VERSION'[[:space:]]*,[[:space:]]*')[^']+(')/\1$NEW_VERSION\2/" "$PLUGIN_FILE"
if grep -q "define([[:space:]]*'LEIRA_TRANSIENTS_VERSION'[[:space:]]*,[[:space:]]*'$NEW_VERSION'" "$PLUGIN_FILE"; then
	echo "Updated LEIRA_TRANSIENTS_VERSION in $PLUGIN_FILE"
else
	echo "❌ Failed to update LEIRA_TRANSIENTS_VERSION in $PLUGIN_FILE"
	exit 1
fi
